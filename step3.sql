...
SELECT * FROM peoples where school not in ‘GDUFS’ or school is null;

#3-2.查找计算机系每次考试所有学生的平均成绩(最终显示考ho名称, 平均分)。
...
update student set student.emotion_state=’single’ where student.emotion_state=’loving’,ID in (select student_ID from exam group by student_ID having avg(grade)<70);
